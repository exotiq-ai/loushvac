...
**Status:** The code is fully wired up. Lou's email (`admin@loushvac.com`) is correctly configured as the recipient. The form degrades gracefully — if email fails but Supabase or HCP captures the lead, users still see a success message.
...
3. In Netlify → Site Settings → Environment Variables, add:
   ...
   - `RESEND_FROM_EMAIL` = `leads@loushvac.com`
   - `RESEND_TO_EMAIL` = `admin@loushvac.com`
